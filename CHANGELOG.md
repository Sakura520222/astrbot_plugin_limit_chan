...
### 重构
- **简化配置结构并优化迁移逻辑**
  - 将 blacklist 和 whitelist 从多平台对象结构简化为用户ID列表
  - 移除 user_configs 和 group_configs 配置项
  - 优化配置迁移逻辑，支持旧格式自动合并到新格式
  - 更新黑名单/白名单查看命令，显示统一列表而非按平台分组
  - 更新配置提示信息，简化用户操作说明
  - 添加 JSON 配置解析工具函数，增强配置容错能力
